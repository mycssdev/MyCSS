...
def main():
    ...
    comp_src = Path(__file__).resolve().parents[2] / "composition" / "src" / "css" / "composition.css"
    composition_css = comp_src.read_text() if comp_src.exists() else ""

    (dist / "mycss-tokens.css").write_text(tokens_css)
    (dist / "mycss-utilities.css").write_text(utilities_css)
    if composition_css:
        (dist / "mycss-composition.css").write_text(composition_css)
    bundle = ("@layer mycss.tokens, mycss.composition, mycss.utilities;\n\n"
              + tokens_css + "\n" + composition_css + "\n" + utilities_css)
    (dist / "mycss.css").write_text(bundle)

    n_vars = tokens_css.count(PREFIX + "")
    ...
    print(f"wrote {dist}/mycss.css ({len(bundle)//1024} KB)")
